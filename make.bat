...
:fmt
    CALL go fmt -mod=mod github.com/ijoywan/laravel-echo-server-4-golang/...
    GOTO :EOF

:static-server
    Set CGO_ENABLED=0
    CALL go install --tags "%BUILDTAGS%" -ldflags "-s -w -extldflags ""-static""" -mod=mod github.com/ijoywan/laravel-echo-server-4-golang/main/laravel-echo-server
    GOTO :EOF

:server
    CALL go install --tags "%BUILDTAGS%" -ldflags "-s -w" -mod=mod github.com/ijoywan/laravel-echo-server-4-golang/main/laravel-echo-server
    GOTO :EOF
...
:clean
    CALL go clean -mod=mod -r github.com/ijoywan/laravel-echo-server-4-golang/...

:run
    Set GOOS=
    Set GOARCH=
    CALL go install -mod=mod github.com/ijoywan/laravel-echo-server-4-golang/main/laravel-echo-server
